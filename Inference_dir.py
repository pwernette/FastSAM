...
def parse_args():
    ...
    parser.add_argument(
        "--img_dir", type=str, default="./images", help="path to image files"
    )
...
def main(args):
    ...
    for ext in exts:
        typeimages = 0
        for f in glob.glob(os.path.join(args.img_dir,'*.'+ext)):
             imglist.append(f)
             typeimages += 1
        print('Found {} images with type {}'.format(typeimages,ext))
    ...
    for i in imglist:
        ...
        prompt_process.plot(
            annotations=ann,
            output_path=args.output+i.split("/")[-1],
            bboxes = bboxes,
            points = points,
